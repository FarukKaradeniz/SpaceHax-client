...
let getStats = (playerName) => {
    req({
        ...
        if (response.data.GamesPlayed > 0) {
            room.sendAnnouncement(`${playerName} ⟹ Gol Sayısı: ${response.data.GoalsCount} 🔹 Asist Sayısı: ${response.data.AssistsCount} 🔹 Oynanan Maç Sayısı: ${response.data.GamesPlayed} 🔹 Kazanılan Maç Sayısı: ${response.data.GamesWon} 🔹 Kazanma Oranı: %${((response.data.GamesWon/response.data.GamesPlayed) * 100).toFixed(2)} 🔹 Maç Başına Gol: ${(response.data.GoalsCount / response.data.GamesPlayed).toFixed(2)}`, undefined, COLOR.YELLOW, "normal", 0);
        } else {
